...
export default function TimelineItem({ experience }: Props) {
  const { name, href, logo, logoDark, positions } = experience;
  ...
  return (
    <li className="relative ml-10 py-4">
      ...
      >
        <div className="relative size-12 rounded-full overflow-hidden border flex items-center justify-center bg-background">
          <Image
            src={logo}
            alt={name}
            fill
            ...
            className={
              logoDark
                ? "block dark:hidden bg-background object-contain"
